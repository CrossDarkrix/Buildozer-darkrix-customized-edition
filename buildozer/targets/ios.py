...
class TargetIos(Target):
    ...
    def __init__(self, buildozer):
        ...
        # run toolchain.py -> kivy_ios.toolchain.main()
        if self.check_kivy_ios_installed() != 1:
            self._toolchain_cmd = [executable, "-c", "from kivy_ios.toolchain import main;main()"]
        else:
            sys.exit(1)
    ...
    def check_kivy_ios_installed(self):
        try:
            import subprocess
            ...
            try:
                ...
                pip(["install", "-UI", "kivy-ios"])
                pip(["install", "-UI", "Cython"])
                return 0
            except:
                return 1
